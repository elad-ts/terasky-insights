...
while read line ; do
  ...
  ACCOUNT_ID=`echo $line | awk '{print $(NF - 2)}'`
  ACCOUNT_JOINT_METHOD=`echo $line | awk '{print $NF}'`
...
connection "aws_${SP_NAME}" {
  plugin  = "aws"
  profile = "$(if [ "$ACCOUNT_JOINT_METHOD" == "INVITED" ]; then echo $SOURCE_PROFILE; else echo "sp_${ACCOUNT_NAME}"; fi)"
  regions = ${ALL_REGIONS}
}

EOF

done < <(aws organizations list-accounts --query "Accounts[?Status!='SUSPENDED'].[Name,Id,Status,JoinedMethod]" --output text --profile $SOURCE_PROFILE)
